...
# FILE: pages/dashboard.py (FOOLPROOF SYNCHRONIZED ARCHITECTURE)
# ====================================================================
...
<script>
    ...
    // Load Streamlit message handlers safely
    ...
    // ⚡ FOOLPROOF COMPONENT HANDSHAKE ENGINE
    function commitClaimToSheets() {
        ...
        // Dispatch write request seamlessly under no-cors constraints
        fetch(pingUrl, { mode: 'no-cors' });
        
        // Give Google Sheets exactly 2.5 seconds to commit the modification, then tell Streamlit to reload data
        setTimeout(() => {
            if (window.parent && window.parent.postMessage) {
                // Standard secure handshake protocol across components
                const syncMsg = {
                    isStreamlitMessage: true,
                    type: "streamlit:setComponentValue",
                    value: "REFRESH_" + timestamp
                };
...
# Render main canvas component and grab its native synchronous signal
sync_signal = st.components.v1.html(html_elements, height=750, scrolling=False)

# 🚀 NATIVE BACKEND INTERCEPTOR: Wipes old data cache and fetches fresh spreadsheet entries
if sync_signal is not None and str(sync_signal).startswith("REFRESH_"):
    st.cache_data.clear()
    st.rerun()        width: 0px !important;
    }
</style>
""", unsafe_allow_html=True)
