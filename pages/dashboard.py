...
# 🔄 RE-FETCH COMMAND ENGINE (Triggers ONLY upon explicit claiming interaction)
if "sync_trigger" in st.session_state:
    st.cache_data.clear()
    del st.session_state["sync_trigger"]

@st.cache_data(ttl=600) # Prevents automatic interval checking loops
...
<script>
    ...
    function commitClaimToSheets() {
        ...
        imgPing.onload = imgPing.onerror = function() {
            setTimeout(() => {
                // ⚡ TRIGGERS THE STATE VALUE VALUE ONLY WHEN PRESSED
...
# Capture execution context only when the value is explicitly populated from the HTML side
