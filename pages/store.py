...
# FIX 1: Read files securely from the repository folder and encode them as direct inline base64 data URIs
def determine_asset_filename(reward_key):
    clean_id = str(reward_key).replace(" ", "")
    # Check possible extensions in the assets/ root directory
    ...
    for path in possible_paths:
        if os.path.exists(path):
            try:
                ...
                pass
    # Fallback to a placeholder icon if the graphic cannot be loaded
...
# FIX 2: Detect the transaction data string in the URL parameters on rerun
...
<script>
    ...
    function executeFinalTransaction() {
        ...
        // Push payload variables through window parent URL structure safely
...
# Build layout grid components with robust embedded Base64 strings
grid_items_html = ""
for item in STORE_ITEMS:
    ...
    <div class="store-card">
        ...
        <div style="width: 100%; display: flex; flex-direction: column; align-items: center;">
            <div class="item-cost-badge">{item['cost']} PTS</div>
...
st.components.v1.html(html_store_elements, height=1050, scrolling=True)        border-color: #718096 !important;
        color: #FFF !important;
    }
</style>
""", unsafe_allow_html=True)
